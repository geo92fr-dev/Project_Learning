...
function generateCompetences(matiere, niveau) {
  ...
  return competencesNames.map((comp, index) => {
    ...
    const isUnlocked = index === 0 || Math.random() > 0.2;
    const objectives = generateObjectives(comp);
    const prerequisites = index > 0 ? generatePrerequisites(competencesNames, index) : [];
    const tags = generateTags(matiere, niveau, comp);
    return {
      id: `comp-${index + 1}`,
      slug: comp.toLowerCase().replace(/[àáâãäå]/g, "a").replace(/[èéêë]/g, "e").replace(/[ìíîï]/g, "i").replace(/[òóôõö]/g, "o").replace(/[ùúûü]/g, "u").replace(/[ç]/g, "c").replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, ""),
      title: comp,
      description: generateDescription(comp, niveau),
      difficulty: index < 2 ? "facile" : index < 4 ? "moyen" : "difficile",
      ...
      progress,
      isUnlocked,
      lastAccessed: hasProgress ? new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1e3) : void 0
    };
  });
}
function generateDescription(competence, niveau) {
  const descriptions = {
...
function generateObjectives(competence, index) {
...
function generatePrerequisites(allCompetences, currentIndex) {
  if (currentIndex === 0)
    return [];
...
function generateTags(matiere, niveau, competence) {
  const tags = [niveau];
...
function generateDuration(index) {
...
function getCompetencesForLevel(matiere, niveau) {
