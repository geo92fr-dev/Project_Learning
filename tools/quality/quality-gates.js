...
class QualityGates {
  ...
  loadConfig() {
    ...
    if (fs.existsSync(this.configPath)) {
      try {
        ...
        console.warn(
          "⚠️ Erreur lecture config, utilisation valeurs par défaut"
        );
  ...
  async executeAll(phase = null) {
    ...
    console.log(
      `📋 Seuils: Coverage ${phaseConfig.coverage.minimum}%, Complexity ≤${phaseConfig.complexity.max}, Security ${phaseConfig.security.level}`
    );
    ...
    const complexityPass = await this.gateCodeComplexity(
      phaseConfig.complexity
    );
    ...
    if (
      phaseConfig.performance &&
      ["phase-2", "phase-3", "phase-4", "phase-5", "phase-6"].includes(
        currentPhase
      )
    ) {
      const performancePass = await this.gatePerformance(
        phaseConfig.performance
      );
  ...
  async gateTestCoverage(coverageConfig) {
    ...
    try {
      ...
      console.log(
        `${status} Couverture: ${coverage}% (min: ${coverageConfig.minimum}%) ${target}`
      );
      ...
      this.results.push({
        ...
        details: passed
          ...
          : `Couverture insuffisante: ${coverage}% < ${coverageConfig.minimum}%`,
      });
  ...
  async gateCodeComplexity(complexityConfig) {
    ...
    try {
      ...
      const result = execSync(
        'npx eslint src/ --format=json --rule "complexity: [2, ' +
          complexityConfig.max +
          ']"',
        {
          encoding: "utf8",
          cwd: this.projectRoot,
      ...
      const complexityErrors = eslintResults.reduce(
        (acc, file) =>
          acc +
          file.messages.filter((msg) => msg.ruleId === "complexity").length,
        0
      );
      ...
      console.log(
        `${status} Complexité: ${complexityErrors} violations (max: ${complexityConfig.max})`
      );
      ...
      this.results.push({
        ...
        details: passed
          ...
          : `${complexityErrors} fonctions trop complexes`,
      });
  ...
  async gateSecurity(securityConfig) {
    ...
    try {
      const auditLevel =
        securityConfig.level === "critical"
          ? "critical"
          : securityConfig.level === "high"
          ? "high"
          : "moderate";
      ...
      const status = passed ? "✅" : "❌";
      console.log(
        `${status} Sécurité: Critical:${criticalCount}, High:${highCount}, Moderate:${moderateCount}`
      );
      ...
      this.results.push({
        ...
        details: passed
          ...
          : `Vulnérabilités détectées niveau ${securityConfig.level}`,
      });
...
${this.results
  .map(
    (r) =>
      `| ${r.gate} | ${r.status} | ${r.value} | ${r.threshold} | ${r.details} |`
  )
  .join("\n")}
...
`;
    ...
    if (allPassed) {
      ...
      console.log(
        "❌ Certaines Quality Gates ont échoué. Voir le rapport pour détails."
      );
    }
  }
