<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      ...
      const { testFirebaseConnection } = await import('$lib/firebase/test-connection-new.js');
...
<main>
  <div class="container">
    ...
    <div class="next-steps">
      ...
      <p>Si ce test réussit, nous pouvons passer à la Phase 2B : Interface d'authentification</p>
    </div>
  </div>
</main>
...
<style>
  ...
  .status-card, .results-card, .next-steps {
    background: white;
    border-radius: 8px;
    padding: 1.5rem;
    margin-bottom: 1rem;
