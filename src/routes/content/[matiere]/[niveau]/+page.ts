...
export const load = async ({ params }: { params: { matiere: string; niveau: string } }) => {
  try {
    const { matiere, niveau } = params;
    
    // Validation des paramètres
    if (!matiere || !niveau || 
        typeof matiere !== 'string' || typeof niveau !== 'string' ||
        matiere.trim() === '' || niveau.trim() === '') {
    ...
    const competences = mockDataValue.competences
      .filter((c: any) => c.matiereId === matiereId && c.niveauId === niveauId);
    ...
    const stats = {
      ...
      difficultesMoyenne: competences.length > 0 
        ? competences.filter((c: any) => c.difficulte === 'moyen').length / competences.length
        : 0
    };
