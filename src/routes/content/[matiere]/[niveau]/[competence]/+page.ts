...
export const load = async ({ params }: { params: { matiere: string; niveau: string; competence: string } }) => {
  try {
    const { matiere, niveau, competence } = params;
    
    // Validation des paramètres
    if (!matiere || !niveau || !competence ||
        typeof matiere !== 'string' || typeof niveau !== 'string' || typeof competence !== 'string' ||
        matiere.trim() === '' || niveau.trim() === '' || competence.trim() === '') {
    ...
    const matiereData = mockDataValue.matieres.find((m: any) => m.id === matiereId);
    if (!matiereData) {
      throw error(404, `Matière "${matiere}" non trouvée`);
    }

    // Vérifier que le niveau existe
    const niveauData = mockDataValue.niveaux.find((n: any) => n.id === niveauId);
    if (!niveauData) {
      throw error(404, `Niveau "${niveau}" non trouvé`);
    }

    // Vérifier que la compétence existe et correspond à la matière/niveau
    const competenceData = mockDataValue.competences.find((c: any) => 
      c.id === competenceId && c.matiereId === matiereId && c.niveauId === niveauId
    );
    if (!competenceData) {
    ...
    const relatedCompetences = mockDataValue.competences
      .filter((c: any) => 
    ...
    const stats = {
      ...
      dureeTotal: courses.reduce((total: number, course: any) => total + course.dureeEstimee, 0),
