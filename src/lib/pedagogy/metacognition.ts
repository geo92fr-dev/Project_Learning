...
export const metacognitionState = writable<
  "idle" | "prompting" | "analyzing" | "completed"
>("idle");
export const reflectionQuality = writable<number>(0);

// Store dérivé pour l'analyse des stratégies
export const strategiesAnalysis = derived([promptHistory], ([$history]) => {
...
const PROMPT_BANK: MetacognitionPrompt[] = [
  // Prompts de planification - Avant l'apprentissage
  {
    ...
    question:
      "Avant de commencer cette leçon, que savez-vous déjà sur ce sujet ?",
    sub_questions: [
    ...
    guidance:
      "Prenez un moment pour réfléchir à vos connaissances actuelles. Cela vous aidera à mieux comprendre les nouveaux concepts.",
    expected_elements: [
...

  {
    ...
    trigger: "before_lesson",
    question:
      "Comment allez-vous organiser votre temps d'apprentissage pour cette session ?",
    sub_questions: [
    ...
    guidance:
      "Une bonne planification améliore l'efficacité de l'apprentissage. Pensez à prévoir du temps pour la révision.",
    expected_elements: [
      "gestion du temps",
  ...
  // Prompts de monitoring - Pendant l'apprentissage
  {
    ...
    guidance:
      "Faire le point régulièrement permet d'identifier les difficultés avant qu'elles s'accumulent.",
...

  {
    ...
    trigger: "during_lesson",
    question:
      "Votre stratégie d'apprentissage fonctionne-t-elle bien pour ce contenu ?",
    sub_questions: [
    ...
    guidance:
      "Adapter sa stratégie en cours de route est une compétence métacognitive importante.",
    expected_elements: [
  ...
  // Prompts d'évaluation - Après l'apprentissage
  {
    ...
    question:
      "Maintenant que vous avez terminé cette leçon, qu'avez-vous retenu ?",
    sub_questions: [
    ...
    guidance:
      "La réflexion après l'apprentissage consolide les connaissances et aide à identifier les lacunes.",
    expected_elements: [
      "synthèse",
...

  {
    ...
    trigger: "after_lesson",
    question:
      "Comment évaluez-vous l'efficacité de votre session d'apprentissage ?",
    sub_questions: [
    ...
    guidance:
      "Évaluer ses méthodes d'apprentissage permet de s'améliorer continuellement.",
  ...
  // Prompts de stratégie - Après exercices
  {
    ...
    question:
      "Comment avez-vous abordé cet exercice ? Décrivez votre démarche.",
    sub_questions: [
    ...
    guidance:
      "Prendre conscience de sa démarche de résolution aide à développer de meilleures stratégies.",
    expected_elements: [
...

  {
    ...
    guidance:
      "La réflexion sur ses propres processus d'apprentissage est au cœur de la métacognition.",
...
export class ResponseAnalyzer {
  ...
  identifyStrategies(content: string): MetacognitiveStrategy[] {
    ...
    if (
      lowerContent.includes("planifier") ||
      lowerContent.includes("organiser") ||
      lowerContent.includes("préparer") ||
      lowerContent.includes("objectif")
    ) {
      strategies.push({
        type: "planning",
    ...
    if (
      lowerContent.includes("vérifier") ||
      lowerContent.includes("contrôler") ||
      lowerContent.includes("surveiller") ||
      lowerContent.includes("suivre")
    ) {
      strategies.push({
        type: "monitoring",
    ...
    if (
      lowerContent.includes("évaluer") ||
      lowerContent.includes("juger") ||
      lowerContent.includes("analyser") ||
      lowerContent.includes("bilan")
    ) {
      strategies.push({
        type: "evaluating",
    ...
    if (
      ...
      lowerContent.includes("difficulté") ||
      lowerContent.includes("corriger")
    ) {
      strategies.push({
        type: "debugging",
    ...
    if (
      lowerContent.includes("réviser") ||
      lowerContent.includes("revoir") ||
      lowerContent.includes("répéter") ||
      lowerContent.includes("reprendre")
    ) {
      strategies.push({
        type: "reviewing",
...
export class MetacognitionService {
  ...
  evaluateAwarenessLevel(
    ...
    for (const response of responses) {
      ...
      if (
        text.includes("parce que") ||
        text.includes("car") ||
        text.includes("donc")
      )
        score += 1; // Justifications
      if (
        text.includes("stratégie") ||
        text.includes("méthode") ||
        text.includes("approche")
      )
        score += 1; // Conscience des stratégies
      if (
        text.includes("difficile") ||
        text.includes("facile") ||
        text.includes("compris")
      )
        score += 1; // Autoévaluation
    }
  ...
  generateAnalysisReport(): any {
    ...
    const qualityScores = history.map((response) => {
      ...
      return prompt
        ? this.analyzer.analyzeResponseQuality(response, prompt)
        : 0;
    });
...
function analyzeMetacognitiveStrategies(
  ...
  for (const strategy of allStrategies) {
    strategyDistribution[strategy.type] =
      (strategyDistribution[strategy.type] || 0) + 1;
  }

  const dominantStrategy =
    Object.entries(strategyDistribution).sort(
      ([, a], [, b]) => b - a
    )[0]?.[0] || "none";

  const averageEffectiveness =
    allStrategies.length > 0
      ? allStrategies.reduce((acc, s) => acc + s.effectiveness, 0) /
...
export function getPromptsByCompetence(
  competence: string
): MetacognitionPrompt[] {
