...
describe("Phase 4 - Pédagogie Avancée", () => {
  describe("AdaptiveEngine", () => {
    ...
    it("devrait calculer correctement la probabilité de maîtrise", () => {
      ...
      const initialProb = engine.calculateMasteryProbability(
        "test-skill",
        [],
        0.5
      );
  ...
  describe("MetacognitionService", () => {
    it("devrait générer des prompts adaptés au niveau", () => {
      const prompts = metacognitionService.generatePromptsForLevel(
        "beginner",
        "mathematics"
      );
    ...
    it("devrait identifier les stratégies d'apprentissage", () => {
      const responses = [
        {
          prompt: "Comment apprenez-vous ?",
          response:
            "Je fais des cartes mentales et je répète les formules pour visualiser",
        },
      ];
    ...
    it("devrait évaluer le niveau de conscience métacognitive", () => {
      const responses = [
        {
          prompt: "Comment apprenez-vous ?",
          response: "Je sais que je comprends mieux avec des exemples visuels",
        },
        {
          prompt: "Que faites-vous quand vous ne comprenez pas ?",
          response:
            "Quand je ne comprends pas, j'essaie de reformuler le problème",
        },
        {
          prompt: "Comment organisez-vous votre étude ?",
          response:
            "Je planifie mon temps d'étude en fonction de la difficulté",
        },
      ];

      const awarenessLevel =
        metacognitionService.evaluateAwarenessLevel(responses);
  ...
  describe("AdaptiveResourceManager", () => {
    ...
    it("devrait adapter le contenu selon le profil", () => {
      ...
      const adaptedContent =
        adaptiveResourceManager.adaptResourceContent(mockResource);
  ...
  describe("Métriques et Analytics Phase 4", () => {
    it("devrait calculer des métriques d'apprentissage valides", () => {
      ...
      const avgSessionTime =
        mockLearningData.totalTimeSpent / mockLearningData.sessionsCompleted;
      const performanceGrade =
        mockLearningData.averageScore >= 0.8 ? "Excellent" : "Bon";
    ...
    it("devrait générer des rapports d'efficacité pédagogique", () => {
      ...
      expect(report.total_resources).toBeGreaterThanOrEqual(
        Object.keys(report.type_breakdown).length
      );
    });
  });
});
...
describe("Tests de performance Phase 4", () => {
  ...
  it("devrait gérer de gros volumes de données de réponse", () => {
    ...
    const startTime = performance.now();
    const analysis = metacognitionService.analyzeReflection(
      largeResponseSet.slice(0, 10)
    );
    const endTime = performance.now();
