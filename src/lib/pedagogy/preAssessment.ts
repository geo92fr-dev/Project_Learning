...
export const assessmentState = writable<'idle' | 'running' | 'completed' | 'analyzing'>('idle');
...
export class AdaptiveEngine {
  ...
  calculateEngagementMetrics(session: {
    ...
    const focus_score = Math.min(1, session.interactions / (session.duration / 60)); // interactions par minute
    const interaction_rate = session.interactions / session.questionsAnswered;
    const completion_rate = session.questionsAnswered / (session.questionsAnswered + 5); // Supposons 5 questions non répondues
   ...
   */
  selectNextQuestion(availableQuestions: PreEvaluationQuestion[]): PreEvaluationQuestion | null {
  ...
  generateAnalysis(): any {
    ...
    // Analytics
    const avgTimePerQuestion = this.responses.length > 0 
      ? this.responses.reduce((acc, r) => acc + r.time_taken, 0) / this.responses.length
      : 0;
  ...
  private calculateConfidenceCorrelation(): number {
    const validResponses = this.responses.filter(r => r.confidence_level !== undefined);
...
export class PreAssessmentManager {
   ...
   */
  async startAssessment(userId: string, targetCompetences?: string[]): Promise<void> {
    this.userId = userId;
   ...
   */
  async submitResponse(response: Omit<PreEvaluationResponse, 'timestamp'>): Promise<boolean> {
  ...
  private getTargetLevel(currentLevel: string): 'beginner' | 'intermediate' | 'advanced' | 'expert' {
...
export function getQuestionsByCompetence(competence: string): PreEvaluationQuestion[] {
