...
// Mock utils module  
vi.mock('../utils', () => ({
  createUserConverter: vi.fn(() => ({ toFirestore: vi.fn(), fromFirestore: vi.fn() })),
  createCourseConverter: vi.fn(() => ({ toFirestore: vi.fn(), fromFirestore: vi.fn() })),
  createUserProgressConverter: vi.fn(() => ({ toFirestore: vi.fn(), fromFirestore: vi.fn() })),
  createCompetenceConverter: vi.fn(() => ({ toFirestore: vi.fn(), fromFirestore: vi.fn() })),
  validateFirestoreData: vi.fn(() => ({ success: true })),
  createTimestamp: vi.fn(() => new Date()),
  generateDocumentId: vi.fn(() => 'test-id'),
  COLLECTION_PATHS: { USERS: 'users', COURSES: 'courses', USER_PROGRESS: 'progress', COMPETENCES: 'competences' },
  handleFirestoreOperation: vi.fn(async (op) => ({ success: true, data: await op() }))
}));
    ...
    it('should update when profile is set', () => {
      const mockProfile = {
        ...
        preferences: {
          ...
          accessibility: { fontSize: 'medium' as const, highContrast: false, reducedMotion: false }
        },
        learningProfile: {
