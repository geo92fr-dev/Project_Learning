...
class AdaptiveAlgorithm {
  ...
  calculateNextDifficulty(
    ...
    const globalAccuracy = this.calculateAccuracy(
      currentResponses,
      questionHistory
    );
    ...
    const currentDifficulty =
      questionHistory[questionHistory.length - 1]?.difficulty || 2;
    const targetDifficulty = Math.max(
      1,
      Math.min(5, currentDifficulty + difficultyAdjustment)
    );
  ...
  private calculateAccuracy(
    ...
    responses.forEach((response, index) => {
      ...
      if (
        question &&
        this.isCorrectAnswer(response.answer, question.correct_answer)
      ) {
        correct++;
      }
    });
...
class QuestionGenerator {
   ...
   */
  private getQuestionsForCompetence(
    competence: string
  ): PreEvaluationQuestion[] {
  ...
  private generateMockQuestions(competence: string): PreEvaluationQuestion[] {
    const baseQuestions: PreEvaluationQuestion[] = [
      ...
      },
      {
        ...
        correct_answer: "60",
        explanation:
          "15 × 4 = 60. On peut calculer 15 × 4 = (10 + 5) × 4 = 40 + 20 = 60.",
      ...
      },
      {
        ...
        correct_answer: "1/2",
        explanation:
          "3/6 peut être simplifiée en divisant le numérateur et le dénominateur par 3 : 3÷3 = 1 et 6÷3 = 2, donc 1/2.",
...
export class PreEvaluationService {
  ...
  async submitResponse(
    ...
    const progress = Math.min(
      100,
      (currentResponses.length / this.config.maxQuestions) * 100
    );
  ...
  private async completeEvaluation(): Promise<void> {
    ...
    evaluation.total_duration =
      evaluation.end_time.getTime() - evaluation.start_time.getTime();
  ...
  private calculateScores(evaluation: PreEvaluationResult): void {
    ...
    evaluation.score = {
      ...
      competence_level:
        this.adaptiveAlgorithm.determineCompetenceLevel(evaluation),
    };
  }
  ...
  private calculateConfidenceCorrelation(
    ...
    const avgConfidence =
      pairs.reduce((sum, p) => sum + p.confidence, 0) / pairs.length;
    const avgCorrect =
      pairs.reduce((sum, p) => sum + p.correct, 0) / pairs.length;
   ...
   */
  private createAdaptiveLearningPath(
    evaluation: PreEvaluationResult
  ): AdaptiveLearningPath {
