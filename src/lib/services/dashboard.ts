...
class DashboardService {
  ...
  public readonly learningStreaks: Writable<{
    current: number;
    longest: number;
  }>;
    ...
    this.currentProfile.subscribe((profile) => {
      if (profile) {
        ...
        const averageProgress =
          competences.reduce((sum, comp) => sum + comp.mastery_percentage, 0) /
          competences.length;
  ...
  private async generateBehavioralInsights(
    ...
    const insights: BehavioralInsights = {
      ...
      attention_patterns: {
        peak_focus_duration:
          profile.learning_analytics.average_session_duration,
  ...
  private async generateRecommendations(
    ...
    const recommendations: ContextualRecommendations = {
      ...
      metacognitive_development:
        this.generateMetacognitiveRecommendations(profile),
    };
  ...
  private calculateMonthlyProgress(profile: LearnerProfile): number {
    ...
    const averageProgression =
      competences.reduce((sum, comp) => sum + comp.progression_rate, 0) /
      competences.length;
  ...
  private calculateAccuracyTrend(profile: LearnerProfile): number[] {
    ...
    return Array.from(
      { length: 7 },
      (_, i) => baseAccuracy + (Math.random() - 0.5) * 0.1
    );
  }
   ...
   */
  private assessChallengePreference(
    profile: LearnerProfile
  ): "low" | "moderate" | "high" | "adaptive" {
    const challengeSeeking = profile.motivation_profile.challenge_seeking;
    const confidence =
      Object.values(profile.competence_levels).reduce(
        (sum, comp) => sum + comp.confidence_level,
        0
  ...
  private generateImmediateActions(
    ...
    Object.entries(profile.competence_levels).forEach(
  ...
  private generateWeeklyGoals(
    ...
    Object.entries(profile.competence_levels).forEach(
      ([competenceId, competence]) => {
        const targetImprovement = Math.min(
          15,
          100 - competence.mastery_percentage
        );
        goals.push({
  ...
  private generateLongTermObjectives(
    ...
    objectives.push({
      ...
      description:
        "Développer vos compétences métacognitives pour un apprentissage plus efficace",
      priority: "high",
   ...
   */
  private generateSkillBuildingRecommendations(
    profile: LearnerProfile
  ): LearningRecommendation[] {
  ...
  private async updateProfileFromSession(
    ...
    this.currentProfile.update((profile) => {
      ...
      analytics.average_session_duration =
        analytics.total_learning_time / analytics.sessions_count;
      ...
      if (sessionDate === today) {
        ...
        analytics.streak_longest = Math.max(
          analytics.streak_longest,
          analytics.streak_current
        );
      }
  ...
  calculateLearningWellness(): number {
    ...
    this.currentProfile.subscribe((profile) => {
      if (profile) {
        ...
        const progress =
          Object.values(profile.competence_levels).reduce(
            (sum, comp) => sum + comp.mastery_percentage,
            0
          ) / Object.keys(profile.competence_levels).length;
        const motivation =
          (profile.motivation_profile.intrinsic_motivation +
            profile.motivation_profile.extrinsic_motivation) *
          10;
        const metacognition =
          (profile.metacognitive_development.self_awareness_level +
            profile.metacognitive_development.strategy_knowledge) *
          12.5;
