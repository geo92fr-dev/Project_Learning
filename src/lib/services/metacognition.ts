...
export const metacognitiveDevelopment = writable<MetacognitiveDevelopment | null>(null);
...
export const metacognitionInsights = derived(
	...
	([$history, $development]) => ({
		...
		averageDepth: $history.length > 0 ? 
			$history.reduce((sum, r) => sum + r.reflection_depth, 0) / $history.length : 0,
...
class MetacognitionPromptBank {
		...
		this.prompts.set('before_lesson', [
			{
				...
				guidance: "Prenez un moment pour réfléchir à vos connaissances existantes. Cela vous aidera à mieux connecter les nouveaux apprentissages.",
				expected_elements: ["connaissances_prealables", "curiosite", "connexions"],
			...
			},
			{
				...
				type: 'strategy',
				trigger: 'before_lesson',
				question: "Quelle stratégie d'apprentissage allez-vous utiliser pour cette leçon ?",
				sub_questions: [
					"Comment allez-vous organiser votre attention ?",
					"Quelles techniques vous aident habituellement à retenir l'information ?"
				],
				guidance: "Choisir une stratégie à l'avance améliore l'efficacité de l'apprentissage.",
				expected_elements: ["strategie_choisie", "organisation", "techniques_memorisation"],
		...
		this.prompts.set('during_lesson', [
			{
				...
				guidance: "Être conscient de sa compréhension en temps réel permet d'ajuster son apprentissage.",
				expected_elements: ["comprehension_actuelle", "points_clairs", "difficultes"],
			...
			},
			{
				...
				guidance: "Ajuster sa stratégie pendant l'apprentissage est une compétence métacognitive clé.",
				expected_elements: ["efficacite_strategie", "ajustements", "techniques_efficaces"],
		...
		this.prompts.set('after_lesson', [
			{
				...
				guidance: "Synthétiser ce qu'on a appris aide à consolider les connaissances.",
			...
			},
			{
				...
				guidance: "Réfléchir sur son propre processus d'apprentissage développe l'autonomie.",
				expected_elements: ["strategies_efficaces", "ameliorations", "decouverte_soi"],
		...
		this.prompts.set('after_exercise', [
			{
				...
				guidance: "Auto-évaluer sa performance développe l'esprit critique et l'autonomie.",
			...
			},
			{
				...
				type: 'strategy',
				trigger: 'after_exercise',
				question: "Quelle stratégie utiliseriez-vous pour un exercice similaire ?",
				sub_questions: [
				...
				guidance: "Planifier des stratégies futures améliore les performances à long terme.",
				expected_elements: ["strategies_futures", "prevention_erreurs", "indices_utiles"],
	...
	selectPrompt(
		...
		const promptsToUse = unusedPrompts.length > 0 ? unusedPrompts : availablePrompts;
	...
	private selectByUserLevel(
		...
		if (level === 'beginner') {
			const simple = prompts.filter(p => p.type === 'self_assessment' || p.type === 'evaluation');
			if (simple.length > 0) return simple[Math.floor(Math.random() * simple.length)];
		}

		// Pour les avancés, privilégier la réflexion et les stratégies
		if (level === 'advanced' || level === 'expert') {
			const complex = prompts.filter(p => p.type === 'reflection' || p.type === 'strategy');
			if (complex.length > 0) return complex[Math.floor(Math.random() * complex.length)];
		}
...
class ResponseAnalyzer {
	...
	private strategyKeywords: Map<string, string[]> = new Map([
		['planning', ['planifier', 'organiser', 'préparer', 'stratégie', 'objectif', 'plan']],
		['monitoring', ['vérifier', 'contrôler', 'surveiller', 'suivre', 'attention', 'comprendre']],
		['evaluating', ['évaluer', 'juger', 'critiquer', 'analyser', 'performance', 'résultat']],
		['debugging', ['corriger', 'réparer', 'erreur', 'problème', 'difficulté', 'obstacle']],
		['reviewing', ['réviser', 'revoir', 'reprendre', 'récapituler', 'synthèse', 'résumé']]
	]);

	// Vocabulaire métacognitif pour mesurer la sophistication
	private metacognitiveVocabulary = [
		'métacognition', 'stratégie', 'réflexion', 'conscience', 'monitoring',
		'autorégulation', 'planification', 'évaluation', 'métaconnaissance',
		'transfert', 'abstraction', 'généralisation', 'conceptualisation'
	];
	 ...
	 */
	analyzeResponse(response: string, prompt: MetacognitionPrompt): Partial<MetacognitionResponse> {
	 ...
	 */
	private calculateReflectionDepth(text: string, prompt: MetacognitionPrompt): 1 | 2 | 3 | 4 | 5 {
		let depth = 1;
		...
		const factors = [
			...
			/\b(parce que|car|donc|ainsi|par conséquent|en effet|cependant|néanmoins)\b/.test(text),
		...
		this.strategyKeywords.forEach((keywords, strategyType) => {
			...
			if (foundKeywords.length > 0) {
				strategies.push({
					...
					effectiveness: this.estimateEffectiveness(foundKeywords.length, text.length),
	 ...
	 */
	private estimateEffectiveness(keywordCount: number, textLength: number): 1 | 2 | 3 | 4 | 5 {
...
export class MetacognitionService {
	...
	async proposePrompt(
		...
		const prompt = this.promptBank.selectPrompt(trigger, competence, userLevel, recentPrompts);
	...
	private async updateMetacognitiveDevelopment(
		...
		if (development.reflection_depth_trend.length > 20) {
			development.reflection_depth_trend = development.reflection_depth_trend.slice(-20);
		}

		// Ajouter nouvelles stratégies découvertes
		response.metacognitive_strategies.forEach(strategy => {
			if (!development!.strategies_discovered.some(s => s.type === strategy.type)) {
	...
	private calculateMetacognitiveSkills(
		...
		const avgDepth = userResponses.reduce((sum, r) => sum + r.reflection_depth, 0) / userResponses.length;
		development.self_awareness_level = Math.min(5, Math.max(1, Math.round(avgDepth))) as 1 | 2 | 3 | 4 | 5;

		// Connaissance des stratégies : basée sur la diversité des stratégies utilisées
		const uniqueStrategies = new Set(
			userResponses.flatMap(r => r.metacognitive_strategies.map(s => s.type))
		);
		development.strategy_knowledge = Math.min(5, Math.max(1, uniqueStrategies.size)) as 1 | 2 | 3 | 4 | 5;
		...
		development.monitoring_skills = Math.min(5, Math.max(1, Math.ceil(monitoringCount / 2))) as 1 | 2 | 3 | 4 | 5;
		...
		development.evaluation_skills = Math.min(5, Math.max(1, Math.ceil(evaluationCount / 2))) as 1 | 2 | 3 | 4 | 5;
	}
	...
	getPersonalizedRecommendations(userId: string): string[] {
		...
		if (development.self_awareness_level < 3) {
			recommendations.push("Prenez du temps pour réfléchir sur votre façon d'apprendre");
		}
		...
		if (development.monitoring_skills < 3) {
			recommendations.push("Surveillez votre compréhension pendant l'apprentissage");
		}
		...
		if (recentDepth.length > 0) {
			const avgRecentDepth = recentDepth.reduce((sum, d) => sum + d, 0) / recentDepth.length;
			if (avgRecentDepth < 3) {
				recommendations.push("Essayez d'approfondir vos réflexions avec plus de détails");
			}
		}
