<script lang="ts">
  ...
  $: isPhotoURLSafe = profile?.photoURL
    ? validateURL(profile.photoURL, {
...
/** * Svelte Component for User Profile Display * Phase 5B: Frontend Integration
- UI Components * * @description Composant Svelte pour afficher et modifier le
profil utilisateur * @follows MyDevFramework CoPilot Best Practices */
    ...
    {#if showPreferences && profile.preferences}
      <div class="preferences-section">
        ...
        <div class="preference-group">
          ...
          {:else}
            <span data-testid="language-display"
              >{profile.preferences.language}</span
            >
          {/if}
        </div>
      </div>
      ...
      <div class="stats-grid">
        <div class="stat-item" data-testid="total-time">
          ...
          <span class="stat-value"
            >{profile.progressTracking.totalTimeSpent} min</span
          >
        </div>
        <div class="stat-item" data-testid="courses-completed">
          <span class="stat-label">Cours terminés</span>
          <span class="stat-value"
            >{profile.progressTracking.coursesCompleted}</span
          >
        </div>
        <div class="stat-item" data-testid="current-streak">
          <span class="stat-label">Série actuelle</span>
          <span class="stat-value"
            >{profile.progressTracking.currentStreak} jours</span
          >
        </div>
      </div>
    </div>
  {:else}
    <div class="loading" data-testid="loading">Chargement du profil...</div>
  {/if}
</div>
