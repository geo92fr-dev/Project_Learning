...
export class QualityDashboard implements vscode.TreeDataProvider<string> {
  private metrics: QualityMetrics;
  private webviewPanel: vscode.WebviewPanel | undefined;
  private readonly refreshInterval = 30000; // 30 secondes
   ...
   */
  private async updateCodeQuality(): Promise<void> {
   ...
   */
  private async updateEducationalQuality(): Promise<void> {
   ...
   */
  private isCBDCompliant(content: string): boolean {
    const cbdPatterns = [
      /\[CONTEXT\]/,
      /\[FILE\]/,
      /\[CMD\]/,
   ...
   */
  private async getSvelteFiles(): Promise<string[]> {
    try {
      const entries = await vscode.workspace.findFiles(
        "**/*.svelte",
        "**/node_modules/**",
...
<body>
  ...
  private getDuplicationStatus(duplication: number): string {
    if (duplication <= 2) return "good";
    if (duplication <= 5) return "warning";
    return "error";
  }

  private getTestStatus(): string {
    const ratio =
      this.metrics.projectHealth.testsPassing /
  ...
  private getLoadTimeStatus(): string {
   ...
   * 📈 Obtenir tendances
   */
  async getTrends(): Promise<QualityTrend[]> {
