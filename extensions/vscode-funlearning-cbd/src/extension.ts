...
import { EducationalAssistant } from "./educational-assistant";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const roadmapTracker = new RoadmapTracker(workspaceRoot);
  const qualityDashboard = new QualityDashboard(workspaceRoot);
  const educationalAssistant = new EducationalAssistant(workspaceRoot);
