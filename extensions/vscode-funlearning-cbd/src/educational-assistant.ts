...
export interface Exercise {
  id: string;
  type: "quiz" | "coding" | "interactive" | "reflection";
  question: string;
  options?: string[];
...
export class EducationalAssistant {
  ...
  private readonly competenceFramework = {
    "6eme": [
  ...
  private analyzeVocabulary(
    ...
    const levelConfig =
      this.vocabularyLevels[level as keyof typeof this.vocabularyLevels];
   ...
   */
  private extractCompetences(content: string, level: string): string[] {
    const levelCompetences =
      this.competenceFramework[
        level as keyof typeof this.competenceFramework
      ] || [];
    const foundCompetences: string[] = [];
   ...
   */
  private getCompetenceKeywords(competence: string): string[] {
   ...
   * 🎮 Analyser engagement
   */
  private analyzeEngagement(content: string): number {
   ...
   * 📊 Générer évaluations
   */
  private generateAssessments(competences: string[]): Assessment[] {
   ...
   */
  private generateContentTemplate(
    topic: string,
    level: string,
    objectives: string[]
  ): string {
