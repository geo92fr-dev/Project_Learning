...
class TemplateManager {
   ...
   */
  async generateTemplate(templateId, variables, context) {
   ...
   */
  async adaptTemplateToContext(template, context) {
   ...
   */
  async generateTemplateFiles(template, variables, context) {
   ...
   */
  procesTemplateFunctions(template, variables, context) {
    let processed = template;
    // Fonction capitalize
    processed = processed.replace(
      /{{capitalize\(([^)]+)\)}}/g,
   ...
   * 🔍 Résoudre variable
   */
  resolveVariable(expression, variables, context) {
    // Variable simple
   ...
   */
  validateCustom(value, validation) {
    try {
   ...
   */
  async showNextSteps(template, context) {
    const steps = [];
   ...
   */
  async openTemplateDocumentation(templateId) {
   ...
   */
  async createTemplateDocumentation(templateId) {
    const template = this.templates.get(templateId);
    if (!template) return;
